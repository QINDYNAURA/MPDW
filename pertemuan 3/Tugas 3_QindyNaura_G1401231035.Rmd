...
- Koefisien $Y_{t−1}$ (0.41149) juga signifikan. Ini adalah **estimasi untuk** $\lambda$. Nilai ini menunjukkan bahwa sekitar 41% dari efek di periode sebelumnya masih "terbawa" ke periode saat ini atau dengan kata lain memengaruhi nilai $Y$ saat ini.
...
BIC(model.ardl)
```
Hasil di atas menunjukkan bahwa peubah $xt_{t}$ sangat signifikan, hal ini menunjukkan bahwa peubah X pada periode saat ini berpengaruh terhadap Y . Hasil uji t untuk $x_{t-7}$ dan $x_{t-8}$ menunjukkan nilai p signifikan mendekati alpa 10%. Model keseluruhannya adalah sebagai berikut:

$$
\hat{Y}=-0.72215 +0.0000794X_t+0.0836X_{t-7}+0.0638Y_{t-8}
$$
```{r}
